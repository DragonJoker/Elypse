...
EMUSE_SCRIPT_FUNCTION_DECLARE( Gen_Return )
{
	...
	if ( ScriptEngine::GetContext()->m_currentFunction != nullptr )
	{
...
EMUSE_SCRIPT_FUNCTION_DECLARE( Gen_BindKey )
{
	...
	if ( p_bindType >= 0 && p_bindType <= 3  && p_keyToBind >= 0 && p_keyToBind < NUM_KEYS && l_code != nullptr )
	{
		if ( ScriptEngine::GetContext()->scriptEngine->m_compiler->m_keyboardBinds[p_bindType][p_keyToBind] != nullptr )
		{
