...
BEGIN_TROLL_GUI_NAMESPACE
{
	...
	void FunctionsTree::CreaFunc( const wxString & p_idfunc, VariableBaseType p_returnValue, std::vector< VariableBaseType > p_params )
	{
		StrPair l_function;
	...
	void FunctionsTree::ClassFunc( const wxString & p_idfunc, VariableBaseType p_returnValue, std::vector< VariableBaseType > p_params )
	{
		StrPair l_function;
		l_function.first = p_idfunc;
		wxString l_data = _buildFunctionHelper( p_idfunc, p_returnValue, p_params );
	...
	{
		auto && l_textCtrl = GUI::MainFrame::GetInstance()->GetCurrentEdit();
