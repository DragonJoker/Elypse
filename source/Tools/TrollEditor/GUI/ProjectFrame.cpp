...
namespace Troll::GUI
{
	...
	namespace
	{
		...
		bool IsElypsePage( wxString const & p_name )
		{
			return p_name.find( TE_PROJECT_TESTING ) == 0
				|| p_name.find( TE_SCENE_EDITION ) == 0
				|| p_name.find( TE_OVERLAYS_EDITION ) == 0
				|| p_name.find( TE_TIMELINE_EDITION ) == 0;
		}
	}
	...
	{
		ProjectSaver::SaveProject( *GetProject() );
	}
	...
	void ProjectFrame::DoInitElypse( ProjectSPtr p_project, wxString const & p_title, bool p_edit, int p_adjustY )
	{
		...
		if ( ( l_size.x < l_projectSize.x || l_size.y < l_projectSize.y )
			&& m_elypseState == ElypseState::ProjectTesting )
		{
	...
	void ProjectFrame::Compile( File const & p_file, Scene const & p_scene )
	{
		...
		else
		{
...

			{
				...
				auto l_it = std::find_if( p_scene.GetLoadScriptFiles().begin()
					, p_scene.GetLoadScriptFiles().end()
					, [&p_file]( File const & l_file )
					{
						return l_file.FileName == p_file.FileName;
					} );
				bool l_found = l_it != p_scene.GetLoadScriptFiles().end();

				for ( auto i = p_scene.GetLoadScriptFiles().begin(); i != l_it; ++i )
				{
					CompileFile( l_compiler, *i );
				}
				...
				if ( !l_found )
				{
					l_it = std::find_if( p_scene.GetUnloadScriptFiles().begin()
						, p_scene.GetUnloadScriptFiles().end()
						, [&p_file]( File const & l_file )
						{
							return l_file.FileName == p_file.FileName;
						} );
					l_found = l_it != p_scene.GetUnloadScriptFiles().end();

					for ( auto i = p_scene.GetUnloadScriptFiles().begin(); i != l_it; ++i )
					{
						CompileFile( l_compiler, *i );
					}
	...
	void ProjectFrame::OnFunctionCopy( wxCommandEvent & p_event )
	{
		p_event.Skip();
	}

	void ProjectFrame::OnFunctionCut( wxCommandEvent & p_event )
	{
		p_event.Skip();
	}

	void ProjectFrame::OnFunctionPaste( wxCommandEvent & p_event )
	{
		p_event.Skip();
	}
