...
{
	auto l_target = m_target
		? m_target->GetAs< SceneNode >()
		: nullptr;
...
void Overlay_Rotate::Apply( Real p_time )
{
	_getRemainingTime( p_time );
	auto l_target = m_target
		? m_target->GetAs< EMOverlayGroup >()->GetOverlay()
		: nullptr;
