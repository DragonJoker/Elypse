...
namespace EMuse
{
	namespace
	{
		...
		void WriteValue( std::string const & p_value, uint8_t p_size, FILE * p_file, size_t & p_nbBytes )
		{
		...
		void ListDirectoryFiles( std::string const & p_folderPath, std::function< void( std::string const & ) > p_function )
		{
#if defined( _WIN32 )
			...
			if ( l_hHandle != INVALID_HANDLE_VALUE )
			{
				...
				while ( ::FindNextFileA( l_hHandle, &l_findData ) )
				{
					if ( l_findData.cFileName != l_strBuffer )
					{
						...
						{
							ListDirectoryFiles( p_folderPath + d_path_slash + l_strBuffer, p_function );
						}
						else
						{
							if ( l_strBuffer != "." && l_strBuffer != ".." )
			...
			else
			{
				...
				while ( ( l_pDirent = readdir( l_pDir ) ) != NULL )
				{
					if ( strcmp( l_pDirent->d_name, "." ) && strcmp( l_pDirent->d_name, ".." ) )
					{
						...
						{
							ListDirectoryFiles( p_folderPath + d_path_slash + std::string( l_pDirent->d_name ), p_function );
						}
						else
						{
...
#	error "Unsupported platform"
		...
		DataBlockType GetType( const std::string & p_filename )
		{
			...
			else if ( l_extension == wxT( "emscene" ) )
			{
	...
	bool DataWriter::CompressFolder( const std::string & p_outFolder, const std::string & p_outPath, DataBlockType p_eType )
	{
		...
		{
			std::string l_outFile = p_outFolder + d_path_slash + GetName( l_file ) + ".zip";
			int l_size = CompressFile( l_file, l_outFile );
			EM_ZipBlock l_block = { l_size, static_cast< uint32_t >( l_file.size() ), l_file, l_outFile };
	...
	bool DataWriter::Write( const std::string & p_filename )
	{
		...
		if ( !m_header.empty() )
		{
			...
			if ( l_outfile )
			{
				...
				std::cout << _( "Header written from byte 8 to byte: " ) << l_nbBytes << std::endl;
	...
	void DataWriter::WriteZipHeader( ZipBlockList p_list, FILE * p_file )
	{
		...
		for ( auto && l_block : p_list )
		{
			...
			Log( "Size", l_block.BlockSize );
			// Writing name size
			WriteValue( l_block.NameSize, p_file, l_nbBytes );
			Log( "Name size", l_block.NameSize );
