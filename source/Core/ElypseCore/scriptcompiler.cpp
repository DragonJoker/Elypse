...
Elypse::Script::ScriptCompiler::ScriptCompiler( const Path & p_path )
	: m_currentLine( 0 )
	, m_path( p_path )
	, m_currentFileStream( nullptr )
	, m_currentUserFunction( nullptr )
	...
	, m_numErrors( 0 )
{
	...
	{
		m_keyboardBinds[0][i] = nullptr;
		m_keyboardBinds[1][i] = nullptr;
		m_keyboardBinds[2][i] = nullptr;
		m_keyboardBinds[3][i] = nullptr;
	}
...
Elypse::Script::ScriptCompiler::~ScriptCompiler()
{
	delete [] m_buffer;
...
//		l_function = _createUserFunction( p_functionName, EMVT_INT);
	...
	if ( l_function->m_finalNode != nullptr )
	{
		return l_function->m_finalNode;
	}

	ScriptNode * l_functionNode = new ScriptNode( nullptr, 0 );
	ScriptNode * l_returnValue = new ScriptNode( nullptr, 0 );
	ScriptNode * l_numParams = new ScriptNode( nullptr, 0 );
...
void Elypse::Script::ScriptCompiler::Initialise()
{
...
{
	if ( m_currentScriptFile != nullptr )
	{
...
char Elypse::Script::ScriptCompiler::_getNextChar()
{
	if ( m_currentFileStream != nullptr )
	{
...
bool Elypse::Script::ScriptCompiler::_eof()
{
	if ( m_currentFileStream != nullptr )
	{
...
void Elypse::Script::ScriptCompiler::_readFile()
{
	m_currentBufferIndex = 0;
...
void Elypse::Script::ScriptCompiler::_putBack( char p_char )
{
	if ( m_currentFileStream != nullptr )
	{
...
{
	m_currentScriptFile = nullptr;
	auto l_line = m_currentLine;
	m_currentLine = 1;
	ScriptBlock * l_initialBlock = new ScriptBlock;
	l_initialBlock->_initialise( this, BT_INITIAL, 0, 0, nullptr );
	m_currentFileStream = nullptr;
	m_currentBufferIndex = 0;
	m_currentMaxIndex = 0;
...
ScriptNode * Elypse::Script::ScriptCompiler::CompileScriptFile( ConfigFile * p_scriptFile )
{
	...
	m_numWarnings = 0;
	m_numErrors = 0;
	ScriptNode * l_scriptNode = nullptr;
	String l_script;
...
ScriptNode * Elypse::Script::ScriptCompiler::_createConstant( VariableBaseType p_type, String const & p_name )
{
	...
	ScriptNode * l_node = CreateScriptNode(); //new ScriptNode( nullptr, 0);
	l_node->SetType( VariableTypeManager::Get( p_type ) );
	l_node->Use();
	m_constants.insert( std::make_pair( p_name, l_node ) );
	return l_node;
}

ScriptBlock * Elypse::Script::ScriptCompiler::_getBlock()
{
...
void Elypse::Script::ScriptCompiler::_releaseBlock( ScriptBlock * p_block )
{
	p_block->Clear();
	m_blockPool.Release( p_block );
}

void Elypse::Script::ScriptCompiler::_leaveUserFunction()
{
...
ScriptNode * Elypse::Script::ScriptCompiler::_createUserVariable( String const & p_variableName, VariableType * p_variableType, bool p_functionParam )
{
	...
	{
		genlib_assert( m_currentUserFunction != nullptr );
		m_currentUserFunction->m_localVars.insert( std::make_pair( p_variableName, l_node ) );
	}
	else
	{
		m_userVariables.insert( std::make_pair( p_variableName, l_node ) );
	}
...
{
	if ( p_functionReturnType == nullptr )
	{
		return nullptr;
	}
...
//	l_function->m_node->m_userFunction = l_function;
	...
	{
		m_userFunctions.insert( std::make_pair( p_functionName, l_function ) );
	}
		...
		VariableBaseType p_left, VariableBaseType p_right )
{
	...
	m_operators.insert( std::make_pair( p_name, l_scriptFunction ) );
}

void Elypse::Script::ScriptCompiler::_initialiseOperatorMap()
{
...
void Elypse::Script::ScriptCompiler::_initialiseVariableMap()
{
...
void Elypse::Script::ScriptCompiler::_initialiseFunctionMap()
{
...
ScriptNode * Elypse::Script::ScriptCompiler::CreateScriptNode()
{
...
ScriptNode * Elypse::Script::ScriptCompiler::GetFlyweight( char p_value )
{
	...
	m_charFlyweight.insert( std::make_pair( p_value, l_node ) );
	return l_node;
}
...
ScriptNode * Elypse::Script::ScriptCompiler::GetFlyweight( String const & p_value )
{
	...
	m_stringFlyweight.insert( std::make_pair( p_value, l_node ) );
	return l_node;
}

ScriptNode * Elypse::Script::ScriptCompiler::GetFlyweight( int p_value )
{
	...
	m_intFlyweight.insert( std::make_pair( p_value, l_node ) );
	return l_node;
}

ScriptNode * Elypse::Script::ScriptCompiler::GetFlyweight( Real p_value )
{
	...
	m_realFlyweight.insert( std::make_pair( p_value, l_node ) );
	return l_node;
}

void Elypse::Script::ScriptCompiler::_releaseNode( ScriptNode * p_node )
{
	p_node->_delete();
...
void Elypse::Script::ScriptCompiler::ReleaseScriptNode( ScriptNode * p_node )
{
