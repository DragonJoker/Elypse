	...
	namespace GUI
	{
			...
			, m_projectFrame( p_parent )
		{
			...
			if ( m_parentOverlay != nullptr )
			{
				...
				if ( l_parentTrollOverlay->GetTextInfos() != nullptr )
				{
		...
		void NewOverlayFrame::OnSelectTemplate( wxCommandEvent & p_event )
		{
			...
			{
				ProjectComponents::Objects2D::TrollOverlay * l_parentTrollOverlay = nullptr;
		...
		void NewOverlayFrame::OnOK( wxCommandEvent & p_event )
		{
			...
			Close();
			m_projectFrame->GetElypseCtrl()->SetNewOverlayFrame( nullptr );
		}
