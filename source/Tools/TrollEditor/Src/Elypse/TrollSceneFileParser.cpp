...
namespace Troll
{
	namespace ElypseRW
	{
		...
		TrollSceneFileParser::TrollSceneFileParser( String const & p_appIndexStr, String const & p_baseDirectory )
		...
		void TrollSceneFileParser::ParseScript( ConfigFile * p_file )
		{
			...
			{
				wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->SetCurrentSceneFile( make_wxString( p_file->GetName() ) );
			}
