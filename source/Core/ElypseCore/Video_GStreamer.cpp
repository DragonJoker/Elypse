...
VideoImpl_GStreamer::~VideoImpl_GStreamer()
{
	...
	if ( m_mediaUrlStr != nullptr )
	{
		delete [] m_mediaUrlStr;
		m_mediaUrlStr = nullptr;
	}
}
...
//		m_fileSrc = gst_element_factory_make( "gnomevfssrc", "fileSource");
	...
	if ( l_bus == nullptr
			|| m_fileSrc == nullptr || m_typeFind == nullptr
			|| m_videoQueue == nullptr || m_audioQueue == nullptr
			|| m_audioSink == nullptr || m_audioConverter == nullptr || m_videoScaleFilter == nullptr
			|| m_videoSink == nullptr || m_videoScaler == nullptr || m_videoConverter == nullptr )
	{
	...
	gst_bin_add_many( GST_BIN( m_pipeline ),
					  ...
					  m_audioQueue, m_audioConverter, m_audioSink, nullptr );
	...
	g_object_set( G_OBJECT( m_fileSrc ), "location", m_mediaUrlStr, nullptr );
	...
	gst_element_link_many( m_audioQueue, m_audioConverter, m_audioSink, nullptr );
}
...
{
	genlib_assert( p_player != nullptr );
	genlib_assert( p_buffer != nullptr );
...
{
	genlib_assert( p_srcPad != nullptr );
	genlib_assert( p_sinkElt != nullptr );
	genlib_assert( p_padName != nullptr );
	...
	for ( ; p_tmplList != nullptr ; p_tmplList = p_tmplList->next )
	{
	...
	if ( g_strrstr( l_mime, "video" ) )
	{
		if ( ! m_videoLinked )
		{
			GstCaps * l_videoScalerCaps = gst_caps_new_simple(
											  ...
											  "bpp", G_TYPE_INT, 32,
											  "depth", G_TYPE_INT, 32,
											  "endianness", G_TYPE_INT, G_BIG_ENDIAN, nullptr );
			g_object_set( G_OBJECT( m_videoScaleFilter ), "caps", l_videoScalerCaps, nullptr );
			g_object_set( G_OBJECT( m_videoScaler ), "method", 1, nullptr );
			g_object_set( G_OBJECT( m_videoSink ), "signal-handoffs", TRUE, "sync", TRUE, "can-activate-pull", TRUE, nullptr );
			g_signal_connect( m_videoSink, "handoff", G_CALLBACK( VideoImpl_GStreamer::_handoffCB ), this );
			gst_caps_unref( l_videoScalerCaps );
			gst_bin_add_many( GST_BIN( m_pipeline ),
							  m_videoQueue, m_videoConverter, m_videoScaler, m_videoScaleFilter, m_videoSink, nullptr );
			gst_element_link_many( m_videoQueue, m_videoConverter, m_videoScaler, m_videoScaleFilter, m_videoSink, nullptr );
...
bool VideoImpl_GStreamer::_tryToPlugAudio( GstPad * p_pad, const GstCaps * p_caps )
{
	...
	if ( l_res != nullptr && ! gst_caps_is_empty( l_res ) )
	{
		_closeLink( p_pad, m_audioQueue, "sink", nullptr );
	...
	for ( const GList * l_item = VideoFactory_GStreamer::GetFactories() ; l_item != nullptr ; l_item = l_item->next )
	{
		GstElementFactory * l_factory = GST_ELEMENT_FACTORY( l_item->data );

		for ( const GList * l_pads = gst_element_factory_get_static_pad_templates( l_factory ) ; l_pads != nullptr ; l_pads = l_pads->next )
		{
			...
			if ( l_res != nullptr && ! gst_caps_is_empty( l_res ) )
			{
				...
				GstElement * l_elt = gst_element_factory_create( l_factory, nullptr );
...
bool VideoImpl_GStreamer::_tryToPlugVideo( GstPad * p_pad, const GstCaps * p_caps )
{
	...
	if ( l_res != nullptr && ! gst_caps_is_empty( l_res ) )
	{
		_closeLink( p_pad, m_videoQueue, "sink", nullptr );
		gst_caps_unref( l_res );
		return true;
	}
	...
	for ( ; l_item != nullptr ; l_item = l_item->next )
	{
		GstElementFactory * l_factory = GST_ELEMENT_FACTORY( l_item->data );
