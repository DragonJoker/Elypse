...
VariableType * EMuse::Script::ScriptCompiler :: _getVariableTypeRecus( ScriptBlockArray & p_blockArray, unsigned int & p_recursIndex )
{
	...
	if ( l_btype == EMVT_MAP )
	{
		...
		if ( l_sub1 != NULL && l_sub1->GetBase() == EMVT_STRING )
		{
			l_btype = EMVT_STRINGMAP;
		}
		else if ( l_sub1 != NULL && l_sub1->GetBase() == EMVT_INT )
		{
			l_btype = EMVT_INTMAP;
		}
		else if ( l_sub1 != NULL && l_sub1->GetBase() == EMVT_REAL )
		{
	...
	{
		if ( l_sub1 == NULL || l_sub2 == NULL )
		{
			l_sub1 = _getVariableTypeRecus( p_blockArray, p_recursIndex );
			l_sub2 = _getVariableTypeRecus( p_blockArray, p_recursIndex );
		}
	}
	else if ( l_btype == EMVT_ITER && l_sub1 == NULL )
	{
		l_sub1 = _getVariableTypeRecus( p_blockArray, p_recursIndex );
