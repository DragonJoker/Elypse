...
{
	genlib_assert( p_object != nullptr );
	Space * l_space = p_object->GetSpace();

	if ( l_space != nullptr && l_space != this )
	{
...
void Space::CollideInternal()
{
	...
	{
		dSpaceCollide( m_space, nullptr, PhysicsEngine::CollisionCallback );
	}
}

void Space::CollideSpace( Space * p_collidesWith )
{
	genlib_assert( p_collidesWith != nullptr );
	dSpaceCollide2( reinterpret_cast <dGeomID>( m_space ), reinterpret_cast <dGeomID>( p_collidesWith->m_space ), nullptr, PhysicsEngine::CollisionCallback );
}

void Space::CollideShape( BoundingShape * p_collidesWith )
{
	genlib_assert( p_collidesWith != nullptr );
	dSpaceCollide2( reinterpret_cast <dGeomID>( m_space ), p_collidesWith->GetGeomID(), nullptr, PhysicsEngine::CollisionCallback );
}
...
void Space::CollideObject( PhysicsObject * p_collidesWith )
{
	...
	{
		dSpaceCollide2( reinterpret_cast <dGeomID>( m_space ), p_collidesWith->GetBoundIndexed( i )->GetGeomID(), nullptr, PhysicsEngine::CollisionCallback );
	}
}
