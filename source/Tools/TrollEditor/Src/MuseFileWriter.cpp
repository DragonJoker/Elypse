...
namespace Troll
{
	...
	void MuseFileWriter::Musinate( Project const & p_project, bool p_createMain, bool p_withScripts )
	{
		...
		if ( l_startScene )
		{
			...
			DoAddSceneToMuse( p_project, p_createMain, p_withScripts, *l_startScene, l_mainFile );

			for ( auto const & l_scene : p_project.GetScenes() )
			{
				if ( l_startScene != l_scene )
				{
					DoAddSceneToMuse( p_project, p_createMain, p_withScripts, *l_scene, l_mainFile );
				}
			}
		}
	}

	void MuseFileWriter::MusinateSceneEditor( Project const & p_current, Project const & p_editor )
	{
		if ( p_editor.HasMainScene() )
		{
			wxString l_mainFile = DoCreateSceneEditorMain( p_current, p_editor );
			DoAddSceneToMuse( p_editor, true, true, *p_editor.GetMainScene(), l_mainFile );
		}
	}

	void MuseFileWriter::MusinateOverlayEditor( Project const & p_current, Project const & p_editor )
	{
		if ( p_editor.HasMainScene() )
		{
	...
	wxString MuseFileWriter::DoCreateMain( Project const & p_project, bool p_writeScripts )
	{
	...
	wxString MuseFileWriter::DoCreateSceneEditorMain( Project const & p_current, ProjectComponents::Project const & p_editor )
	{
		wxString l_mainFileName;
	...
	wxString MuseFileWriter::DoCreateOverlayEditorMain( Project const & p_current, ProjectComponents::Project const & p_editor )
	{
		wxString l_mainFileName;
		l_mainFileName = p_editor.GetPath() + wxT( "main.emcfg" );
		wxFileOutputStream l_fileOutput( l_mainFileName );
	...
	void MuseFileWriter::DoAddSceneToMuse( Project const & p_project, bool p_createMain, bool p_withScripts, Scene const & p_scene, wxString const & p_mainFile )
	{
		...
		if ( p_scene.GetName() == p_project.GetMainScene()->GetName() && p_createMain )
		{
	...
	void MuseFileWriter::DoZipFolder( wxString const & p_folderPath, wxString const & p_zipFileName )
	{
