...
BEGIN_TROLL_PROJECT_2D_NAMESPACE
{
	...
	int TrollOverlay::BuildPanel( wxWindow * p_parent, int p_width )
	{
		...
		m_propertiesPanel = new TROLL_GUI_PROPERTIES_NAMESPACE::OverlayProperties( this, p_parent, wxPoint( 0, 0 ), l_returnSize );
		return l_height;
	}
	...
	//	std::cout << "TrollOverlay::Write - " << ScriptEngine::GetContext()->mainViewport->getActualWidth() << "x" << ScriptEngine::GetContext()->mainViewport->getActualHeight() << " - " << GUI::MainFrame::GetInstance()->m_testPanel->GetClientSize().x << "x" << GUI::MainFrame::GetInstance()->m_testPanel->GetClientSize().y << "\n";
		if ( m_parent != NULL )
		{
