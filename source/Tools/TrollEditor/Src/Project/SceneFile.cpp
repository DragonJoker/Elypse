...
BEGIN_TROLL_PROJECT_NAMESPACE
{
	...
	Objects3D::TrollEntity * SceneFile::GetEntity( const wxString & p_name )const
	{
		return General::Utils::map::findOrNull( m_entities, p_name );
	}

	Objects2D::TrollOverlay * SceneFile::GetOverlay( const wxString & p_name )const
	{
		return General::Utils::map::findOrNull( m_allOverlays, p_name );
	}

	Objects3D::TrollSceneNode * SceneFile::GetSceneNode( const wxString & p_name )const
	{
		return General::Utils::map::findOrNull( m_allSceneNodes, p_name );
	}

	Objects3D::TrollLight * SceneFile::GetLight( const wxString & p_name )const
	{
		return General::Utils::map::findOrNull( m_lights, p_name );
	}

	Objects3D::TrollCamera * SceneFile::GetCamera( const wxString & p_name )const
	{
		return General::Utils::map::findOrNull( m_cameras, p_name );
	}

	Temporal::TrollAnimationGroup * SceneFile::GetAnimationGroup( const wxString & p_name )const
	{
		return General::Utils::map::findOrNull( m_animationGroups, p_name );
	}

	Media::TrollSound * SceneFile::GetSound( const wxString & p_name )const
	{
		return General::Utils::map::findOrNull( m_sounds, p_name );
	}

	Media::TrollVideo * SceneFile::GetVideo( const wxString & p_name )const
	{
		return General::Utils::map::findOrNull( m_videos, p_name );
	}

	Temporal::TrollSequence * SceneFile::GetSequence( const wxString & p_name )const
	{
		return General::Utils::map::findOrNull( m_sequences, p_name );
	}

	void SceneFile::_writeUnusedFilePieces( wxTextOutputStream * p_textStream )
	{
		String l_strFilePath = make_string( GUI::MainFrame::GetInstance()->GetAppDir() + m_fileName + wxT( "Unused.emscene" ) );
	...
	void SceneFile::Write( const wxString & p_path )
	{
