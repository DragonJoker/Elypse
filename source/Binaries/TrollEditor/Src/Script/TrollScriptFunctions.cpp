...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Ent_SetMaterial )
{
	...
	if ( MaterialManager::getSingletonPtr()->getByName( l_materialName ).isNull() == false )
	{
		...
		if ( l_entity != NULL )
		{
			...
			if ( g_mainFrame->IsEditingScene() && g_mainFrame->GetCurrentProject()->GetMainScene()->GetEntity( l_entity->getName() ) != NULL )
			{
				g_mainFrame->GetCurrentProject()->GetMainScene()->GetEntity( l_entity->getName() )->SetMaterial( l_materialName.c_str(), false );
			}
		}
		else
		{
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_OvE_SetHeight )
{
	...
	if ( p_overlay != NULL )
	{
		...
		TrollOverlay * l_teOv = g_mainFrame->GetCurrentProject()->GetMainScene()->GetOverlay( p_overlay->GetName() );
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_OvE_SetCaption )
{
	...
	if ( p_overlay != NULL )
	{
		...
		if ( g_mainFrame->IsEditingOverlays() && l_teOv != NULL )
		{
			l_teOv->SetCaption( l_caption.c_str(), false );
		}
	}
	else
	{
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_OvE_SetMouseOverMaterial )
{
	...
	if ( l_overlay != NULL )
	{
		...
		if ( MaterialManager::getSingletonPtr()->getByName( l_materialName ).isNull() == false )
		{
			...
			TrollOverlay * l_teOv = g_mainFrame->GetCurrentProject()->GetMainScene()->GetOverlay( l_overlay->GetName() );

			if ( g_mainFrame->IsEditingOverlays() && l_teOv != NULL )
			{
				l_teOv->SetMouseOverMaterial( l_materialName.c_str(), false );
			}
		}
		else
		{
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_OvE_SetMaterial )
{
	...
	if ( l_overlay != NULL )
	{
		...
		if ( ! MaterialManager::getSingletonPtr()->getByName( l_materialName ).isNull() )
		{
			...
			if ( g_mainFrame->IsEditingOverlays() && l_teOv != NULL )
			{
				l_teOv->SetBaseMaterial( l_materialName.c_str(), false );
			}
		}
		else
		{
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_ScN_RotateAround )
{
	...
	if ( l_node != NULL )
	{
		...
		if ( l_angle != 0.0 )
		{
			...
			if ( g_mainFrame->IsEditingScene() && g_mainFrame->GetCurrentProject()->GetMainScene()->GetSceneNode( l_node->getName() ) != NULL )
			{
				g_mainFrame->GetCurrentProject()->GetMainScene()->GetSceneNode( l_node->getName() )->SetOrientation( l_node->getOrientation(), false );
				g_mainFrame->GetCurrentProject()->GetMainScene()->GetSceneNode( l_node->getName() )->SetPosition( l_node->getPosition(), false );
			}
		}
	}
	else
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_ScN_Scale )
{
	...
	if ( l_node != NULL )
	{
		...
		if ( l_scale != Vector3( 1.0, 1.0, 1.0 ) )
		{
			...
			if ( g_mainFrame->IsEditingScene() && g_mainFrame->GetCurrentProject()->GetMainScene()->GetSceneNode( l_node->getName() ) != NULL )
			{
				g_mainFrame->GetCurrentProject()->GetMainScene()->GetSceneNode( l_node->getName() )->SetScale( l_node->getScale(), false );
			}
		}
	}
	else
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_ScN_SetScale )
{
	...
	if ( l_node != NULL )
	{
		...
		if ( g_mainFrame->IsEditingScene() && g_mainFrame->GetCurrentProject()->GetMainScene()->GetSceneNode( l_node->getName() ) != NULL )
		{
			g_mainFrame->GetCurrentProject()->GetMainScene()->GetSceneNode( l_node->getName() )->SetScale( l_scale, false );
		}
	}
	else
	{
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_ScN_Reset )
{
	...
	if ( l_node != NULL )
	{
		...
		if ( g_mainFrame->IsEditingScene() && g_mainFrame->GetCurrentProject()->GetMainScene()->GetSceneNode( l_node->getName() ) != NULL )
		{
			g_mainFrame->GetCurrentProject()->GetMainScene()->GetSceneNode( l_node->getName() )->SetScale( Vector3::UNIT_SCALE, false );
			g_mainFrame->GetCurrentProject()->GetMainScene()->GetSceneNode( l_node->getName() )->SetOrientation( Quaternion::IDENTITY, false );
		}
	}
	else
	{
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Cam_ModeWireFrame )
{
	...
	if ( l_camera != NULL )
	{
		if ( l_bool )
		{
			if ( g_mainFrame->IsEditingScene() && g_mainFrame->GetCurrentProject()->GetMainScene()->GetCamera( l_camera->getName() ) != NULL )
			{
				g_mainFrame->GetCurrentProject()->GetMainScene()->GetCamera( l_camera->getName() )->SetPolygonMode( Troll::Objects3D::tcWireFrame );
			}
		}
		else
		{
			if ( g_mainFrame->IsEditingScene() && g_mainFrame->GetCurrentProject()->GetMainScene()->GetCamera( l_camera->getName() ) != NULL )
			{
				g_mainFrame->GetCurrentProject()->GetMainScene()->GetCamera( l_camera->getName() )->SetPolygonMode( Troll::Objects3D::tcSolid );
			}
		}
	}
	else
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Cam_SetPosition )
{
	...
	if ( l_camera != NULL )
	{
		...
		if ( g_mainFrame->IsEditingScene() && g_mainFrame->GetCurrentProject()->GetMainScene()->GetCamera( l_camera->getName() ) != NULL )
		{
			g_mainFrame->GetCurrentProject()->GetMainScene()->GetCamera( l_camera->getName() )->SetPosition( l_position, false );
		}
	}
	else
	{
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Lgh_SetDirection )
{
	...
	if ( l_light != NULL )
	{
		...
		if ( g_mainFrame->IsEditingScene() && g_mainFrame->GetCurrentProject()->GetMainScene()->GetLight( l_light->getName() ) != NULL )
		{
			g_mainFrame->GetCurrentProject()->GetMainScene()->GetLight( l_light->getName() )->SetDirection( l_sLightDirection, false );
		}
	}
	else
	{
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Lgh_SetPosition )
{
	...
	if ( l_sLight != NULL )
	{
		...
		if ( g_mainFrame->IsEditingScene() && g_mainFrame->GetCurrentProject()->GetMainScene()->GetLight( l_sLight->getName() ) != NULL )
		{
			g_mainFrame->GetCurrentProject()->GetMainScene()->GetLight( l_sLight->getName() )->SetPosition( l_sLightPosition, false );
		}
	}
	else
	{
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Lgh_SetDiffuse )
{
	...
	if ( l_sLight != NULL )
	{
		...
		if ( g_mainFrame->IsEditingScene() && g_mainFrame->GetCurrentProject()->GetMainScene()->GetLight( l_sLight->getName() ) != NULL )
		{
			g_mainFrame->GetCurrentProject()->GetMainScene()->GetLight( l_sLight->getName() )->SetDiffuse( l_sLightDiffuse, false );
		}
	}
	else
	{
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Lgh_SetSpecular )
{
	...
	if ( l_sLight != NULL )
	{
		...
		if ( g_mainFrame->IsEditingScene() && g_mainFrame->GetCurrentProject()->GetMainScene()->GetLight( l_sLight->getName() ) != NULL )
		{
			g_mainFrame->GetCurrentProject()->GetMainScene()->GetLight( l_sLight->getName() )->SetSpecular( l_sLightSpecular, false );
		}
	}
	else
	{
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Spc_ImportMesh )
{
	...
	wxFileDialog l_dialog( NULL, "Choisissez un mesh à ouvrir", "", "", "Mesh Files (*.mesh)|*.mesh|",
						   wxFD_OPEN | wxFD_FILE_MUST_EXIST );

	if ( l_dialog.ShowModal() == wxID_OK )
	{
		l_path = l_dialog.GetPath();
		...
		if ( ! FileCopy( ( ScriptEngine::GetContext()->lastDataRep + wxString( wxFileName::GetPathSeparator() ).char_str().data() + l_meshName ).c_str(), ( g_mainFrame->GetCurrentProject()->GetPath() + l_file->FileName + wxFileName::GetPathSeparator() + l_meshName.c_str() ).char_str().data() ) )
		{
		...
		for ( unsigned short i = 0 ; i < l_nbSubmeshes ; i++ )
		{
			...
			if ( l_material != NULL )
			{
				if ( l_material->getOrigin() != l_fileName )
				{
					...
					if ( ! FileCopy( ScriptEngine::GetContext()->lastDataRep + "/" + l_fileName, ( g_mainFrame->GetCurrentProject()->GetPath() + l_file->FileName + "/" + l_fileName.c_str() ).char_str().data() ) )
					{
				...
				for ( unsigned short j = 0 ; j < l_nbTechniques ; j++ )
				{
					...
					for ( unsigned short k = 0 ; k < l_nbPasses ; k++ )
					{
						...
						for ( unsigned short l = 0 ; l < l_nbTextureUnits ; l++ )
						{
							...
							if ( ! FileCopy( ScriptEngine::GetContext()->lastDataRep + "/" + l_textureUnit->getTextureName(), ( g_mainFrame->GetCurrentProject()->GetPath() + l_file->FileName + "/" + l_textureUnit->getTextureName().c_str() ).char_str().data() ) )
							{
