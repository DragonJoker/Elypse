	...
	namespace ElypseRW
	{
		...
		TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Ent_SetMaterial )
		{
			...
			if ( MaterialManager::getSingletonPtr()->getByName( l_materialName ) )
			{
				...
				if ( l_entity != NULL )
				{
					...
					if ( wxGetApp().GetMainFrame()->IsEditingScene() && wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetEntity( make_wxString( l_entity->getName() ) ) != NULL )
					{
		...
		TROLL_SCRIPT_FUNCTION_DECLARE( Troll_OvE_SetHeight )
		{
			...
			if ( p_overlay != NULL )
			{
				...
				if ( wxGetApp().GetMainFrame()->IsEditingOverlays() && l_teOv != NULL )
				{
		...
		TROLL_SCRIPT_FUNCTION_DECLARE( Troll_ScN_RotateAround )
		{
			...
			if ( l_node != NULL )
			{
				...
				if ( l_angle != 0.0 )
				{
					...
					if ( wxGetApp().GetMainFrame()->IsEditingScene() && wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetSceneNode( make_wxString( l_node->getName() ) ) != NULL )
					{
		...
		TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Cam_ModeWireFrame )
		{
			...
			if ( l_camera != NULL )
			{
				if ( l_bool )
				{
					if ( wxGetApp().GetMainFrame()->IsEditingScene() && wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetCamera( make_wxString( l_camera->getName() ) ) != NULL )
					{
		...
		TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Lgh_SetDirection )
		{
			...
			if ( l_light != NULL )
			{
				...
				if ( wxGetApp().GetMainFrame()->IsEditingScene() && wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetLight( make_wxString( l_light->getName() ) ) != NULL )
				{
		...
		TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Lgh_SetPosition )
		{
			...
			if ( l_sLight != NULL )
			{
				...
				if ( wxGetApp().GetMainFrame()->IsEditingScene() && wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetLight( make_wxString( l_sLight->getName() ) ) != NULL )
				{
		...
		{
			if ( wxGetApp().GetMainFrame()->GetProject() != NULL )
			{
