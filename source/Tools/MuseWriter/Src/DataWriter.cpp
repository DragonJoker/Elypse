...
namespace EMuse
{
	...
	bool DataWriter::AddFolder( const wxString & p_folder, Download::DataBlockType p_eType )
	{
		m_fileslist.clear();
		...
		ListFiles( p_folder );
		wxString l_path = l_folder + wxFileName::GetPathSeparator() + l_foldername + Download::DataTypeExtension[p_eType];
	...
	bool DecompressFile( const wxString & p_zipFileName, const wxString & p_fileName, int p_size )
	{
		...
		if ( l_return )
		{
			...
			if ( l_nbBytesRead < 0 )
			{
				...
				{
				case Z_ERRNO:
					std::cout << "Can't inflate file [" + l_fileName + "] due to error " + std::to_string( errno ) << std::endl;
					break;

				case Z_STREAM_END:
					std::cout << "Can't inflate file [" + l_fileName + "] due to Z_STREAM_END error" << std::endl;
					break;

				case Z_DATA_ERROR:
					std::cout << "Can't inflate file [" + l_fileName + "] due to Z_DATA_ERROR error" << std::endl;
					break;

				case Z_STREAM_ERROR:
					std::cout << "Can't inflate file [" + l_fileName + "] due to Z_STREAM_ERROR error" << std::endl;
					break;

				case Z_NEED_DICT:
					std::cout << "Can't inflate file [" + l_fileName + "] due to Z_NEED_DICT error" << std::endl;
					break;

				case Z_MEM_ERROR:
					std::cout << "Can't inflate file [" + l_fileName + "] due to Z_MEM_ERROR error" << std::endl;
					break;
				}
			}
	...
	void DataWriter::WriteZipFile( ZipBlockList p_list, FILE * p_file )
	{
		for ( ZipBlockList::const_iterator l_it = p_list.begin(); l_it != p_list.end(); ++l_it )
		{
			...
			General::Files::FileDelete( p_block->CompleteURL.char_str().data() );
		}
	}
	...
	void DataWriter::WriteHeader( unsigned long long & l_nbBytes )
	{
		...
		for ( int i = 0;i < l_nbBlocks;i++ )
		{
			...
			m_log->WriteText( wxString( wxT( "WriteHeader - block hash :\t\t" ) ) + wxString( block->m_hash, wxConvLibc ) + wxString( wxT( "\n" ) ) );
	...
	{
		wxString l_fileUrl = p_fileurl;
		l_fileUrl.Replace( wxT( "/" ), wxString() + wxFileName::GetPathSeparator() );
		l_fileUrl.Replace( wxT( "\\" ), wxString() + wxFileName::GetPathSeparator() );
	...
	wxString DataWriter::GetParentFolder( const wxString & p_folder )
	{
		...
		l_folder.Replace( wxT( "\\" ), wxString() + wxFileName::GetPathSeparator() );
		l_folder.Replace( wxT( "/" ), wxString() + wxFileName::GetPathSeparator() );
