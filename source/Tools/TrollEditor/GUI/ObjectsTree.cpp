	...
	namespace GUI
	{
		...
		void ObjectsTree::AddSceneNode( wxString const & p_name, wxString const & p_parentName )
		{
			...
			else
			{
				...
				RecursiveTreeItemObject * l_parentItem = nullptr;

				while ( l_it != m_sceneNodes.end() && ( l_parentItem = l_it->second->RecursiveIsIn( p_parentName ) ) == nullptr )
				{
					++l_it;
				}
		...
		ProjectComponents::Objects2D::TrollOverlay * ObjectsTree::AddOverlay( ProjectComponents::Objects2D::TrollOverlay * p_overlay, wxString const & p_parentName )
		{
			...
			else
			{
				...
				while ( l_it != m_overlays.end() && ( l_parentItem = l_it->second->RecursiveIsIn( p_parentName ) ) == nullptr )
				{
					++l_it;
				}
		...
		void ObjectsTree::OnObjectLClic( wxTreeEvent & p_event )
		{
			...
			if ( l_itemType != ITEM_DESC_FOLDER )
			{
				...
				{
					if ( ( l_elem = l_it2->second->RecursiveIsIn( l_item.m_pItem ) ) != nullptr )
					{
