	...
	, m_appIndexStr( p_appIndexStr )
{
	...
	m_rootParsers					["<startup_script>"] = Parser_RootStartupScript;
	m_rootParsers					["</startup_script>"] = Parser_RootStartupScriptEnd;
...
bool SceneFileParser::_parseScriptLine( String & p_line )
{
	...
	case SS_STARTUP_SCRIPT:
	{
		if ( p_line == "</startup_script>" )
		{
			Parser_RootStartupScriptEnd( p_line, m_context );
		}
		else
		{
...
//	FileStreamDataStream * FSDSstream = new FileStreamDataStream( l_stream, true);
	...
	m_context->baseDirectory = l_path.GetPath();
