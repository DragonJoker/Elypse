...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Ent_SetMaterial )
{
	...
	if ( MaterialManager::getSingletonPtr()->getByName( l_materialName ).isNull() == false )
	{
		...
		if ( l_entity != NULL )
		{
			...
			if ( g_mainFrame->IsEditingScene() && g_mainFrame->GetCurrentProject()->GetMainScene()->GetEntity( wxString( l_entity->getName().c_str(), wxConvLibc ) ) != NULL )
			{
				g_mainFrame->GetCurrentProject()->GetMainScene()->GetEntity( wxString( l_entity->getName().c_str(), wxConvLibc ) )->SetMaterial( wxString( l_materialName.c_str(), wxConvLibc ), false );
			}
		}
		else
		{
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_OvE_SetHeight )
{
	...
	if ( p_overlay != NULL )
	{
		...
		TrollOverlay * l_teOv = g_mainFrame->GetCurrentProject()->GetMainScene()->GetOverlay( wxString( p_overlay->GetName().c_str(), wxConvLibc ) );

		if ( g_mainFrame->IsEditingOverlays() && l_teOv != NULL )
		{
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_OvE_SetCaption )
{
	...
	if ( p_overlay != NULL )
	{
		...
		if ( g_mainFrame->IsEditingOverlays() && l_teOv != NULL )
		{
			l_teOv->SetCaption( wxString( l_caption.c_str(), wxConvLibc ), false );
		}
	}
	else
	{
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_OvE_SetMouseOverMaterial )
{
	...
	if ( l_overlay != NULL )
	{
		...
		if ( MaterialManager::getSingletonPtr()->getByName( l_materialName ).isNull() == false )
		{
			...
			TrollOverlay * l_teOv = g_mainFrame->GetCurrentProject()->GetMainScene()->GetOverlay( wxString( l_overlay->GetName().c_str(), wxConvLibc ) );

			if ( g_mainFrame->IsEditingOverlays() && l_teOv != NULL )
			{
				l_teOv->SetMouseOverMaterial( wxString( l_materialName.c_str(), wxConvLibc ), false );
			}
		}
		else
		{
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_OvE_SetMaterial )
{
	...
	if ( l_overlay != NULL )
	{
		...
		if ( ! MaterialManager::getSingletonPtr()->getByName( l_materialName ).isNull() )
		{
			...
			if ( g_mainFrame->IsEditingOverlays() && l_teOv != NULL )
			{
				l_teOv->SetBaseMaterial( wxString( l_materialName.c_str(), wxConvLibc ), false );
			}
		}
		else
		{
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_ScN_RotateAround )
{
	...
	if ( l_node != NULL )
	{
		...
		if ( l_angle != 0.0 )
		{
			...
			if ( g_mainFrame->IsEditingScene() && g_mainFrame->GetCurrentProject()->GetMainScene()->GetSceneNode( wxString( l_node->getName().c_str(), wxConvLibc ) ) != NULL )
			{
				g_mainFrame->GetCurrentProject()->GetMainScene()->GetSceneNode( wxString( l_node->getName().c_str(), wxConvLibc ) )->SetOrientation( l_node->getOrientation(), false );
				g_mainFrame->GetCurrentProject()->GetMainScene()->GetSceneNode( wxString( l_node->getName().c_str(), wxConvLibc ) )->SetPosition( l_node->getPosition(), false );
			}
		}
	}
	else
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_ScN_Scale )
{
	...
	if ( l_node != NULL )
	{
		...
		if ( l_scale != Vector3( 1.0, 1.0, 1.0 ) )
		{
			...
			if ( g_mainFrame->IsEditingScene() && g_mainFrame->GetCurrentProject()->GetMainScene()->GetSceneNode( wxString( l_node->getName().c_str(), wxConvLibc ) ) != NULL )
			{
				g_mainFrame->GetCurrentProject()->GetMainScene()->GetSceneNode( wxString( l_node->getName().c_str(), wxConvLibc ) )->SetScale( l_node->getScale(), false );
			}
		}
	}
	else
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_ScN_SetScale )
{
	...
	if ( l_node != NULL )
	{
		...
		if ( g_mainFrame->IsEditingScene() && g_mainFrame->GetCurrentProject()->GetMainScene()->GetSceneNode( wxString( l_node->getName().c_str(), wxConvLibc ) ) != NULL )
		{
			g_mainFrame->GetCurrentProject()->GetMainScene()->GetSceneNode( wxString( l_node->getName().c_str(), wxConvLibc ) )->SetScale( l_scale, false );
		}
	}
	else
	{
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_ScN_Reset )
{
	...
	if ( l_node != NULL )
	{
		...
		if ( g_mainFrame->IsEditingScene() && g_mainFrame->GetCurrentProject()->GetMainScene()->GetSceneNode( wxString( l_node->getName().c_str(), wxConvLibc ) ) != NULL )
		{
			g_mainFrame->GetCurrentProject()->GetMainScene()->GetSceneNode( wxString( l_node->getName().c_str(), wxConvLibc ) )->SetScale( Vector3::UNIT_SCALE, false );
			g_mainFrame->GetCurrentProject()->GetMainScene()->GetSceneNode( wxString( l_node->getName().c_str(), wxConvLibc ) )->SetOrientation( Quaternion::IDENTITY, false );
		}
	}
	else
	{
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Cam_ModeWireFrame )
{
	...
	if ( l_camera != NULL )
	{
		if ( l_bool )
		{
			if ( g_mainFrame->IsEditingScene() && g_mainFrame->GetCurrentProject()->GetMainScene()->GetCamera( wxString( l_camera->getName().c_str(), wxConvLibc ) ) != NULL )
			{
				g_mainFrame->GetCurrentProject()->GetMainScene()->GetCamera( wxString( l_camera->getName().c_str(), wxConvLibc ) )->SetPolygonMode( Troll::Objects3D::tcWireFrame );
			}
		}
		else
		{
			if ( g_mainFrame->IsEditingScene() && g_mainFrame->GetCurrentProject()->GetMainScene()->GetCamera( wxString( l_camera->getName().c_str(), wxConvLibc ) ) != NULL )
			{
				g_mainFrame->GetCurrentProject()->GetMainScene()->GetCamera( wxString( l_camera->getName().c_str(), wxConvLibc ) )->SetPolygonMode( Troll::Objects3D::tcSolid );
			}
		}
	}
	else
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Cam_SetPosition )
{
	...
	if ( l_camera != NULL )
	{
		...
		if ( g_mainFrame->IsEditingScene() && g_mainFrame->GetCurrentProject()->GetMainScene()->GetCamera( wxString( l_camera->getName().c_str(), wxConvLibc ) ) != NULL )
		{
			g_mainFrame->GetCurrentProject()->GetMainScene()->GetCamera( wxString( l_camera->getName().c_str(), wxConvLibc ) )->SetPosition( l_position, false );
		}
	}
	else
	{
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Lgh_SetDirection )
{
	...
	if ( l_light != NULL )
	{
		...
		if ( g_mainFrame->IsEditingScene() && g_mainFrame->GetCurrentProject()->GetMainScene()->GetLight( wxString( l_light->getName().c_str(), wxConvLibc ) ) != NULL )
		{
			g_mainFrame->GetCurrentProject()->GetMainScene()->GetLight( wxString( l_light->getName().c_str(), wxConvLibc ) )->SetDirection( l_sLightDirection, false );
		}
	}
	else
	{
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Lgh_SetPosition )
{
	...
	if ( l_sLight != NULL )
	{
		...
		if ( g_mainFrame->IsEditingScene() && g_mainFrame->GetCurrentProject()->GetMainScene()->GetLight( wxString( l_sLight->getName().c_str(), wxConvLibc ) ) != NULL )
		{
			g_mainFrame->GetCurrentProject()->GetMainScene()->GetLight( wxString( l_sLight->getName().c_str(), wxConvLibc ) )->SetPosition( l_sLightPosition, false );
		}
	}
	else
	{
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Lgh_SetDiffuse )
{
	...
	if ( l_sLight != NULL )
	{
		...
		if ( g_mainFrame->IsEditingScene() && g_mainFrame->GetCurrentProject()->GetMainScene()->GetLight( wxString( l_sLight->getName().c_str(), wxConvLibc ) ) != NULL )
		{
			g_mainFrame->GetCurrentProject()->GetMainScene()->GetLight( wxString( l_sLight->getName().c_str(), wxConvLibc ) )->SetDiffuse( l_sLightDiffuse, false );
		}
	}
	else
	{
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Lgh_SetSpecular )
{
	...
	if ( l_sLight != NULL )
	{
		...
		if ( g_mainFrame->IsEditingScene() && g_mainFrame->GetCurrentProject()->GetMainScene()->GetLight( wxString( l_sLight->getName().c_str(), wxConvLibc ) ) != NULL )
		{
			g_mainFrame->GetCurrentProject()->GetMainScene()->GetLight( wxString( l_sLight->getName().c_str(), wxConvLibc ) )->SetSpecular( l_sLightSpecular, false );
		}
	}
	else
	{
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Spc_SaveScript )
{
	...
	{
		g_mainFrame->GetCurrentProject()->Write();
	}
}
...
TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Spc_ImportMesh )
{
	...
	{
		l_path = l_dialog.GetPath().char_str().data();
		...
		TrollFile * l_file = g_mainFrame->GetCurrentProject()->GetMainScene()->GetDataFile( 0 );
		...
		String l_source = ScriptEngine::GetContext()->lastDataRep + String( wxString( wxFileName::GetPathSeparator() ).char_str().data() ) + l_meshName;
		String l_destination = String( ( g_mainFrame->GetCurrentProject()->GetPath() + l_file->FileName + wxFileName::GetPathSeparator() ).char_str().data() ) + l_meshName;
		...
		for ( unsigned short i = 0 ; i < l_nbSubmeshes ; i++ )
		{
			...
			if ( l_material != NULL )
			{
				if ( l_material->getOrigin() != l_fileName )
				{
					...
					String l_destination = ( wxString( g_mainFrame->GetCurrentProject()->GetPath() ) << l_file->FileName << wxString( wxFileName::GetPathSeparator() ) ).char_str().data() + l_fileName;
				...
				for ( unsigned short j = 0 ; j < l_nbTechniques ; j++ )
				{
					...
					for ( unsigned short k = 0 ; k < l_nbPasses ; k++ )
					{
						...
						for ( unsigned short l = 0 ; l < l_nbTextureUnits ; l++ )
						{
							...
							String l_destination = ( wxString( g_mainFrame->GetCurrentProject()->GetPath() ) << l_file->FileName << wxString( wxFileName::GetPathSeparator() ) ).char_str().data() + l_textureUnit->getTextureName();
