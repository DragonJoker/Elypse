...
namespace Troll
{
	...
	namespace GUI
	{
		...
		ScenePtr FilesTree::DoGetSceneForItem( wxTreeItemId const & p_item )
		{
			...
			if ( l_it == m_scenes.end() )
			{
				LogError( wxT( "No Scene with this item ID." ) );
				return nullptr;
			}
		...
		void FilesTree::OnNewScene( wxCommandEvent & p_event )
		{
			try
			{
				...
				auto l_project = m_projectFrame->GetProject();
				auto l_scene = std::make_shared< Scene >( *l_project, l_sceneName );

				if ( l_project->AddScene( l_scene ) )
				{
					AddSceneToProject( l_scene );
				}
			}
			catch ( std::exception const & p_exc )
			{
		...
		void FilesTree::OnAddExistingFile( wxCommandEvent & p_event )
		{
			...
			if ( l_scene )
			{
				...
				if ( !filename.empty() )
				{
					...
					File & l_file = l_scene->AddFile( l_fileType, text, true, true );
