...
{
	void MusePackDeleter::operator()( MusePack * ptr )const noexcept
	{
		delete ptr;
	}
		...
		, m_beginTime{}
	{
		...
		EMUSE_MESSAGE_DEBUG( "MusePack::MusePack -> checkheader : " + m_checkHeader );
	}
	...
	bool MusePack::_getHeaders( size_t & p_indexBuffer )
	{
		...
		while ( p_indexBuffer < m_waitingDatas.size() && m_files.size() < m_nbHeader )
		{
			if ( !l_file )
			{
				m_files.push_back( MuseFilePackPtr( new MuseFilePack( *this ) ) );
				l_file = m_files.back().get();
			}
		...
		m_headerSize = p_indexBuffer;
		EMUSE_MESSAGE_RELEASE( "MusePack(" + std::to_string( uintptr_t( this ) ) + ")::GetHeaders for [" + GetOwner()->GetOwner()->GetName() + "], " + std::to_string( m_headerSize ) );
	...
	size_t MusePack::CurlCallback( void * p_buffer, size_t p_size )
	{
		if ( !m_timer )
		{
			m_timer = std::make_unique< PreciseTimer >();
		...
		while ( l_indexBuffer < m_waitingDatas.size() )
		{
			...
			size_t l_offset{ m_headerSize };

			for ( size_t i = 0; l_indexBuffer < m_waitingDatas.size() && i < l_nbHeader; i++ )
			{
				auto & l_file = *m_files[i];
				auto l_curDownloaded = l_file.GetDownloadedBytes();
				auto l_curFinished = l_file.IsDownloadFinished();

				if ( !l_file.ReadContents( l_indexBuffer, m_waitingDatas ) )
				{
	...
	bool MusePack::_treatGZipFile( const Path & p_basePath, String const & p_fileName, String const & p_folderName )
	{
		...
		{
			EMUSE_MESSAGE_RELEASE( "MusePack::_treatGZipFile : File[" + p_basePath / p_fileName + "] doesn't exist, can't carry on..." );
			return false;
		}

		if ( !DirectoryExists( p_basePath / p_folderName ) )
		{
			EMUSE_MESSAGE_RELEASE( "MusePack::_treatGZipFile : Directory[" + p_basePath / p_folderName + "] doesn't exist, creating it..." );
