...
bool VideoImpl_DirectShow::Start()
{
	...
	//if ( m_mediaControl == nullptr || FAILED( m_mediaControl->Run() ) )
	{
		return false;
	}
...
{
	{
		...
		if ( ! m_ready /*|| m_mediaControl == nullptr*/ )
		{
...
bool VideoImpl_DirectShow::Stop()
{
	...
	//m_mediaSeeking->SetPositions( & l_position, AM_SEEKING_AbsolutePositioning | AM_SEEKING_SeekToKeyFrame, nullptr, AM_SEEKING_NoPositioning );
	return true;
}
...
bool VideoImpl_DirectShow::Update( Real p_time )
{
	...
	if ( m_playTime >= m_mediaTime )
	{
		if ( GetOwner()->IsLooped() )
		{
			...
			//__int64 l_position = 0;
			//m_mediaSeeking->SetPositions( & l_position, l_flags, nullptr, AM_SEEKING_NoPositioning );
			m_playTime = 0.0;
		}
		else
		{
...
bool VideoImpl_DirectShow::DoGrabFrame()
{
	...
	//{
	//	EMUSE_LOG_MESSAGE_NORMAL( "CVideoTexture::frameStarted - nullptr pointer argument." );
	//}
