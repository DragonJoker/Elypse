...
{
	if ( p_context->universe == nullptr )
	{
		PARSING_ERROR( "Parsing Error : Parser_StartScene -> no universe" );
		return false;
	}

	if ( p_context->universe->GetViewport() == nullptr )
	{
...
DEFINE_SCENE_FILE_PARSER( Parser_RootScene )
{
	...
	if ( p_context->scene == nullptr )
	{
		ZonePtr l_zone = std::make_unique< Zone >( p_params, *p_context->universe );
		p_context->scene = l_zone.get();
		p_context->universe->AddZone( std::move( l_zone ) );
	}
...
DEFINE_SCENE_FILE_PARSER( Parser_RootObject )
{
	...
	p_context->object = nullptr;
	p_context->subObject = nullptr;
...
DEFINE_SCENE_FILE_PARSER( Parser_RootSound )
{
	...
	p_context->soundInstance = nullptr;
	/*
	p_context->general_string_a = p_params;
...
DEFINE_SCENE_FILE_PARSER( Parser_RootVideo )
{
	...
	if ( p_context->videoObject != nullptr )
	{
...
DEFINE_SCENE_FILE_PARSER( Parser_RootStartupScript )
{
	p_context->section = SS_STARTUP_SCRIPT;
...
DEFINE_SCENE_FILE_PARSER( Parser_RootStartupScriptEnd )
{
	p_context->section = SS_NONE;
	p_context->emuseInstance->SetStartupScript( p_context->general_string_a );
	p_context->general_string_b.clear();
	p_context->general_string_a.clear();
	return false;
}
...
{
	if ( p_context->sceneManager == nullptr )
	{
		return false;
	}
...
DEFINE_SCENE_FILE_PARSER( Parser_RootPortal )
{
	...
	if ( p_context->scene == nullptr )
	{
		p_context->portal = nullptr;
		return true;
	}

	p_context->portal = p_context->scene->GetPortal( p_params );

	if ( p_context->portal == nullptr )
	{
...
DEFINE_SCENE_FILE_PARSER( Parser_AnimationGroupElement )
{
	...
	if ( p_context->animatedObjectGroup == nullptr )
	{
...
{
	p_context->section = SS_NONE;
	p_context->animatedObjectGroup = nullptr;
	return false;
}
...
DEFINE_SCENE_FILE_PARSER( Parser_AnimationObject )
{
	...
	if ( p_context->animatedObject == nullptr )
	{
	...
	if ( p_context->animation == nullptr )
	{
...
{
	p_context->section = SS_ANIMATED_OBJECT_GROUP;
	p_context->animatedObject = nullptr;
	return false;
}
...
DEFINE_SCENE_FILE_PARSER( Parser_ObjectAttach )
{
	if ( !p_context->object )
	{
		PARSING_ERROR( "Parser_AttachObject -> null object" );
		return false;
	}
...
{
	if ( p_context->subObject == nullptr )
	{
...
DEFINE_SCENE_FILE_PARSER( Parser_SceneNodeParent )
{
	...
	if ( p_context->sceneNode->getParentSceneNode() != nullptr )
	{
...
DEFINE_SCENE_FILE_PARSER( Parser_SceneNodeEnd )
{
	...
	if ( p_context->sceneNode != nullptr && p_context->sceneNode->getParentSceneNode() == nullptr )
	{
...
DEFINE_SCENE_FILE_PARSER( Parser_CameraTex )
{
	...
	if ( p_context->camTexManager->GetElementByName( p_context->camera->getName() ) != nullptr )
	{
...
//	OverlayContainer * cont;
	...
	if (p_context->overlay == nullptr)
	{
		if (OverlayManager::getSingleton().getOverlayElement( p_params, true) != nullptr )
		{
	...
	{
		if( p_context->gui->GetElementByName( p_context->general_string_a) == nullptr )
		{
...
{
	if ( p_context->overlay == nullptr || p_context->overlay->GetOgreOverlayElement() == nullptr )
	{
...
DEFINE_SCENE_FILE_PARSER( Parser_OverlayPosition )
{
	...
	{
		if ( p_context->overlay->GetOgreOverlayElement()->getParent() != nullptr )
		{
...
{
	if ( p_context->overlayGroup != nullptr )
	{
...
DEFINE_SCENE_FILE_PARSER( Parser_SoundURL )
{
	...
	if ( p_context->soundObject != nullptr )
	{
...
DEFINE_SCENE_FILE_PARSER( Parser_SoundGlobal )
{
	...
	if ( p_context->soundObject != nullptr && p_params == "false" )
	{
...
DEFINE_SCENE_FILE_PARSER( Parser_SoundTimer )
{
	...
	if ( p_context->soundObject != nullptr && p_context->soundObject->GetGlobal() != nullptr )
	{
...
DEFINE_SCENE_FILE_PARSER( Parser_SoundVolumePercent )
{
	...
	else if ( p_context->soundInstance != nullptr )
	{
...
{
	if ( p_context->particleSystem == nullptr )
	{
...
DEFINE_SCENE_FILE_PARSER( Parser_ParticleSystemEnd )
{
	...
	p_context->section = SS_NONE;
	p_context->particleSystem = nullptr;
	return false;
}
...
{
	p_context->section = SS_NONE;
	p_context->physicsMaterial = nullptr;
	return false;
}
...
{
	if ( p_context->physicsObject == nullptr )
	{
		return false;
	}
...
DEFINE_SCENE_FILE_PARSER( Parser_ObjectPhysicsEnd )
{
	...
	p_context->physicsObject = nullptr;
	return false;
}
...
{
	p_context->section = SS_OBJECT_PHYSICS;
	p_context->boundingShape = nullptr;
	return false;
}
...
DEFINE_SCENE_FILE_PARSER( Parser_BillboardEnd )
{
	...
	p_context->section = SS_NONE;
	p_context->billboardSet = nullptr;
	return false;
}

DEFINE_SCENE_FILE_PARSER( Parser_ClickableShapeTranslate )
{
	if ( p_context->clickableShape == nullptr )
	{
...
{
	p_context->clickableShape = nullptr;
	p_context->section = SS_OVERLAY;
	return false;
}
...
//	p_context->log->logMessage( "DEFINE_SCENE_FILE_PARSER( Parser_SequenceTarget)");
	...
	if ( l_split[0] == "scene_node" )
	{
		...
		if ( p_context->sequenceTarget == nullptr )
		{
