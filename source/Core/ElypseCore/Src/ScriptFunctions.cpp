...
EMUSE_SCRIPT_FUNCTION_DECLARE( Tmr_CreatePermanent )
{
	...
	if ( ! l_name.empty() )
	{
		RETURN_AS( ScriptTimer * ) ScriptEngine::GetContext()->timerManager->AddTimer( l_name, 0.0, l_code, EMTT_PERMANENT, NULL );
	}
	else
	{
...
EMUSE_SCRIPT_FUNCTION_DECLARE( OvE_Create )
{
	...
	if ( ScriptEngine::GetContext()->gui->GetElementByName( l_name ) == NULL )
	{
...
EMUSE_SCRIPT_FUNCTION_DECLARE( OvE_SetFontSize )
{
	...
	if ( p_overlay != NULL && p_overlay->GetOgreOverlayElement() != NULL && p_overlay->GetOgreOverlayElement()->getTypeName() == "TextArea" )
	{
...
EMUSE_SCRIPT_FUNCTION_DECLARE( Vid_CreateInstance )
{
	...
	if ( l_object != NULL && p_entity != NULL )
	{
...
EMUSE_SCRIPT_FUNCTION_DECLARE( Vid_ChangeVideo )
{
	...
	if ( l_sEnt == NULL
			|| VideoManager::GetSingletonPtr()->GetVideoObject( l_sVideoObjectName ) == NULL )
	{
		return;
	}
...
#	elif ELYPSE_LINUX
	...
	l_dialog = reinterpret_cast <GtkFileChooser *>(
				   gtk_file_chooser_dialog_new( "Open Mesh File(s)", NULL,
...
EMUSE_SCRIPT_FUNCTION_DECLARE( Spc_GetMuseDirectory )
{
...
EMUSE_SCRIPT_FUNCTION_DECLARE( Spc_Download )
{
	VERBOSE_STARTFUNC( "Spc_Download" );
	GET_AND_EXEC_PARAM( String, p_name, 0 );
...
EMUSE_SCRIPT_FUNCTION_DECLARE( Sce_Load )
{
	...
	if ( l_zone != NULL )
	{
		ScriptEngine::GetContext()->scriptEngine->LoadZone( l_zone );
	}
	else
	{
...
EMUSE_SCRIPT_FUNCTION_DECLARE( AoG_GetObject )
{
	...
	{
		RETURN_AS( AnimatedObject * ) NULL;
		SCRIPT_ERROR( "Error @ AnimationGroup_GetObject : animation group NULL" );
	}
}
...
EMUSE_SCRIPT_FUNCTION_DECLARE( AoG_AddObject )
{
	...
	{
		RETURN_AS( bool ) false;
		SCRIPT_ERROR( "Error @ AnimationGroup_AddObject : animation group or entity NULL" );
	}
}
...
EMUSE_SCRIPT_FUNCTION_DECLARE( AoG_RemObject )
{
	...
	if ( l_group != NULL )
	{
		if ( l_group->GetObjectByName( l_name ) == NULL )
		{
	...
	{
		SCRIPT_ERROR( "Error @ AnimationGroup_RemoveObject : animation group NULL" );
	}
}
...
EMUSE_SCRIPT_FUNCTION_DECLARE( AOb_GetAnimation )
{
	...
	{
		SCRIPT_ERROR( "Error @ AnimatedObject_GetAnimation : animated object NULL" );
		RETURN_AS( EMAnimation * ) NULL;
	}
}
...
EMUSE_SCRIPT_FUNCTION_DECLARE( AOb_GetPlayingAnimationName )
{
	...
	{
		SCRIPT_ERROR( "Error @ AOb_GetPlayingAnimationName : animated object NULL" );
		RETURN_AS( String ) EMPTY_STRING;
	}
}
...
EMUSE_SCRIPT_FUNCTION_DECLARE( POb_GetSpeed )
{
	...
	{
		SCRIPT_ERROR( "POb_GetSpeed : PhysicsObject NULL" );
	}
}
...
EMUSE_SCRIPT_FUNCTION_DECLARE( POb_SetSpeed )
{
	...
	{
		SCRIPT_ERROR( "POb_SetSpeed : PhysicsObject NULL" );
	}
}
...
EMUSE_SCRIPT_FUNCTION_DECLARE( POb_GetAngularSpeed )
{
	...
	{
		SCRIPT_ERROR( "POb_GetAngularSpeed : PhysicsObject NULL" );
	}
}
...
EMUSE_SCRIPT_FUNCTION_DECLARE( POb_SetAngularSpeed )
{
	...
	{
		SCRIPT_ERROR( "POb_SetAngularSpeed : PhysicsObject NULL" );
	}
}
...
EMUSE_SCRIPT_FUNCTION_DECLARE( POb_AddForce )
{
	...
	{
		SCRIPT_ERROR( "POb_AddForce : PhysicsObject NULL" );
	}
}
...
EMUSE_SCRIPT_FUNCTION_DECLARE( POb_SetForce )
{
	...
	{
		SCRIPT_ERROR( "POb_SetForce : PhysicsObject NULL" );
	}
}
...
EMUSE_SCRIPT_FUNCTION_DECLARE( POb_AddTorque )
{
	...
	{
		SCRIPT_ERROR( "POb_AddTorque : PhysicsObject NULL" );
	}
}
...
EMUSE_SCRIPT_FUNCTION_DECLARE( POb_SetTorque )
{
	...
	{
		SCRIPT_ERROR( "POb_SetTorque : PhysicsObject NULL" );
	}
}
...
EMUSE_SCRIPT_FUNCTION_DECLARE( POb_SetIgnoreGravity )
{
	...
	{
		SCRIPT_ERROR( "POb_SetIgnoreGravity : PhysicsObject NULL" );
	}
}
...
EMUSE_SCRIPT_FUNCTION_DECLARE( POb_GetIgnoreGravity )
{
	...
	{
		SCRIPT_ERROR( "POb_GetIgnoreGravity : PhysicsObject NULL" );
	}
}
...
EMUSE_SCRIPT_FUNCTION_DECLARE( POb_SetEnabled )
{
	...
	{
		SCRIPT_ERROR( "POb_SetEnabled : PhysicsObject NULL" );
	}
}
...
EMUSE_SCRIPT_FUNCTION_DECLARE( POb_IsEnabled )
{
	...
	{
		SCRIPT_ERROR( "POb_IsEnabled : PhysicsObject NULL" );
	}
}
...
EMUSE_SCRIPT_FUNCTION_DECLARE( POb_Reset )
{
	...
	{
		SCRIPT_ERROR( "POb_Reset : PhysicsObject NULL" );
	}
}
...
EMUSE_SCRIPT_FUNCTION_DECLARE( PhS_SetCurrentSimulation )
{
	...
	if ( ScriptEngine::GetContext()->physicsSimulation == NULL )
	{
