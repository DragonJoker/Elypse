...
EMUSE_SCRIPT_FUNCTION_DECLARE( Ent_GetParent )
{
	...
	{
		SCRIPT_ERROR( "Error @ Entity_GetParent : Entity nullptr" );
...
EMUSE_SCRIPT_FUNCTION_DECLARE( Ent_GetBoundingRadius )
{
	...
	{
		SCRIPT_ERROR( "Error @ Entity_GetBoundingRadius : Entity nullptr" );
		RETURN_AS( Real ) 0.0;
	}
}
...
EMUSE_SCRIPT_FUNCTION_DECLARE( Ent_SetParent )
{
	...
	if ( l_node )
	{
		if ( l_object->getParentSceneNode() != nullptr )
		{
...
EMUSE_SCRIPT_FUNCTION_DECLARE( ScN_AttachTo )
{
	...
	{
		if ( l_sceneNode2 != nullptr )
		{
			if ( l_sceneNode->getParentSceneNode() != nullptr )
			{
