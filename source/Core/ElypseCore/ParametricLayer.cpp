	...
	, m_quadrant( p_quadrant )
{
	...
	PrecalcCoefficients();
	m_subLayers[0] = nullptr;
	m_subLayers[1] = nullptr;
	m_subLayers[2] = nullptr;
	m_subLayers[3] = nullptr;
	m_points[0] = nullptr;
	m_points[1] = nullptr;
	m_points[2] = nullptr;
	m_points[3] = nullptr;
...
{
	if ( m_points[0] == nullptr )
	{
		CreateSurface( 0 );
	}

	if ( m_points[1] == nullptr )
	{
		CreateSurface( 1 );
	}

	if ( m_points[2] == nullptr )
	{
		CreateSurface( 2 );
	}

	if ( m_points[3] == nullptr )
	{
		CreateSurface( 3 );
	}
...
{
	if ( m_subLayers[p_quadrant] == nullptr )
	{
...
bool ParametricLayer::UpdateResolution( Camera * p_camera )
{
	...
	{
		if ( m_points[l_quadrant] != nullptr )
		{
...
void ParametricLayer::RemoveQuadrant( uint32_t p_quadrant )
{
	if ( m_subLayers[p_quadrant] != nullptr )
	{
		...
		m_subLayers[p_quadrant] = nullptr;
	}
}
