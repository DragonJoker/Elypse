...
namespace General::Files
{
	...
	namespace
	{
		long GetCurlProtocol( UrlProtocol protocol )
		{
			switch ( protocol )
			{
	...
	CURLcode CURLManager::OpenUrl( Url const & p_url, size_t( *p_function )( void *, size_t, size_t, void * ), void * p_data, std::string const & p_postParams )
	{
		...
		do
		{
			...
			l_returnValue = curl_easy_setopt( m_handle, CURLOPT_PROTOCOLS, GetCurlProtocol( p_url.GetProtocol() ) );
	...
	CURLcode CURLManager::GetStringFromUrl( Url const & p_url, std::string & p_contents, std::string const & p_postParams )
	{
		return OpenUrl( p_url, &CURLManager::doGetString, &p_contents, p_postParams );
	}

	CURLcode CURLManager::DownloadFile( Url const & p_url, std::string const & p_filename, std::string const & p_postParams )
	{
		std::ofstream l_file;
	...
	std::string CURLManager::GetContentType( Url const & p_url, std::string const & p_postParams )
	{
