...
// Specific to Troll
	if ( m_initTree )
	{
		...
		va_end( l_argsList );
		g_mainFrame->CreateFreeFunction( wxString( p_functionName ), p_returnValue, l_params );
	}

	if ( m_textCtrl )
	{
		( *m_textCtrl->GetContext()->Find( wxT( "emscript" ) ) )->AddWord( eSTC_TYPE_WORD3, wxString( p_functionName ) );
	}
...
// Specific to Troll
	if ( m_initTree )
	{
		...
		va_end( l_argsList );
		g_mainFrame->CreateClassFunction( wxString( p_functionName ), p_returnValue, l_params );
	}
...
// Specific to Troll
	...
	if ( m_initTree )
	{
		g_mainFrame->CreateConstant( wxString( p_name ), p_type );
	}

	if ( m_textCtrl )
	{
		( *m_textCtrl->GetContext()->Find( wxT( "emscript" ) ) )->AddWord( eSTC_TYPE_WORD2, wxString( p_name ) );
	}
...
void TrollScriptCompiler :: _functionGroup( const String & p_name )
{
	if ( m_initTree )
	{
		g_mainFrame->CreateFunctionsGroup( static_cast <wxString>( p_name ) );
	}
}
...
void TrollScriptCompiler :: _constantGroup( const String & p_name )
{
	if ( m_initTree )
	{
		g_mainFrame->CreateConstantsGroup( static_cast <wxString>( p_name ) );
	}
}
