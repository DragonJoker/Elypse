...
#	define PARSE_WARNING( p_strWarning) GUI::FileParser::ParseWarning( (p_strWarning), (p_pContext) )
		...
		*/
		...
		{
		private:
			using ParserFunctionAndParams = std::pair< PParserFunction, ParameterTypeArray >;

			struct AttributeParserMap
			{
			private:
				using MapType = std::map< wxString, ParserFunctionAndParams >;
			...
			public:
				...
				decltype( auto ) find( wxString const & p_key )const
				{
					return m_map.find( p_key );
				}

				decltype( auto ) find( wxString const & p_key )
				{
					return m_map.find( p_key );
				}
				...
				decltype( auto ) insert( MapType::value_type const & p_pair )
				{
					return m_map.insert( p_pair );
				}

				decltype( auto ) operator[]( wxString const & p_key )
				{
					return m_map[p_key];
				}
