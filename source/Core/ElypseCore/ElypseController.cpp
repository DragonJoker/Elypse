...
namespace
{
	char const * const c_installWebsiteUrl = "https://www.dragonjoker.org/Elypse/Install/";
...
{
	genlib_assert( p_instance != nullptr );
...
void ElypseController::AddThread( ElypseInstance & p_owner )
{
	...
	if ( !GetSingletonPtr() )
	{
		EMUSE_MESSAGE_RELEASE( "ElypseController::AddThread // Problem : sm_singleton == nullptr" );
		return;
	}
...
void ElypseController::Initialise()
{
	...
	if ( Root::getSingletonPtr() == nullptr )
	{
...
void ElypseController::InitialiseRessources( String const & p_prefix )
{
	...
	if ( ! p_prefix.empty() )
	{
		...
		if ( !MaterialManager::getSingletonPtr()->getByName( "DefaultSettings" ) )
		{
		...
		if ( !MaterialManager::getSingletonPtr()->getByName( "BaseWhiteNoLighting" ) )
		{
...
//		m_currentOwner->GetPlugin()->UnlockGui();
		...
		if ( VideoManager::GetSingletonPtr() != nullptr )
		{
			VideoManager::GetSingletonPtr()->Update();
		}
...
//		ElypseResourceGroupManager::destroyResourceGroup( "core.zip");
		...
		if ( m_primaryRenderWindow != nullptr )
		{
