...
int32_t NPP_WriteReady( NPP instance, NPStream * stream )
{
	...
	uint32_t rv = plugin->WriteReady( stream );
	return rv;
}
...
int32_t NP_LOADDS NPP_Write( NPP instance, NPStream * stream, int32_t offset, int32_t len, void * buffer )
{
	...
	uint32_t rv = plugin->Write( stream, offset, len, buffer );
	return rv;
}
...
NPError	Private_New( NPMIMEType pluginType, NPP instance, uint16_t mode, int16 argc, char * argn[], char * argv[], NPSavedData * saved )
{
...
uint32_t Private_WriteReady( NPP instance, NPStream * stream )
{
	uint32_t rv = NPP_WriteReady( instance, stream );
	return rv;
}

uint32_t Private_Write( NPP instance, NPStream * stream, uint32_t offset, uint32_t len, void * buffer )
{
	uint32_t rv = NPP_Write( instance, stream, offset, len, buffer );
	return rv;
}
