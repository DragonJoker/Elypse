...
namespace Troll
{
	...
	namespace GUI
	{
		namespace
		{
			...
			{
			public:
				LogStreambuf( wxString const & p_prefix
					, wxLog * p_logFile
					, wxLog * p_logPanel )
					: m_prefix( p_prefix )
					, m_logPanel( p_logPanel )
					, m_logFile( p_logFile )
				{
				}
					...
					if ( !m_buffer.empty() )
					{
						if ( wxIsMainThread() )
						{
							if ( m_buffer != wxT( "\n" ) )
							{
								if ( m_logPanel )
								{
									wxLog::SetActiveTarget( m_logPanel );
									wxLogMessage( m_buffer );
								}

								if ( m_logFile )
								{
									wxLog::SetActiveTarget( m_logFile );
									wxLogMessage( m_buffer );
								}
							}
						}
			...
			, m_debug{ p_debug }
		{
			...
			auto l_binPath = l_installPath.GetPath();
			l_installPath = l_binPath.GetPath() / "share" / "TrollEditor";
			m_appPath = make_wxString( l_userPath );
			wxCopyDir( make_wxString( l_installPath ), m_appPath );

			DoInitialise();

			m_fileLog = new LogFile( l_binPath / "TrollEditor.log" );
			wxLog::SetActiveTarget( m_logMessage );
			m_oldCout = std::cout.rdbuf( new LogStreambuf( wxT( "[  ]" ), m_fileLog, m_logMessage ) );
			m_oldCerr = std::cerr.rdbuf( new LogStreambuf( wxT( "[EE]" ), m_fileLog, m_logError ) );

			if ( m_debug )
			{
				m_oldClog = std::clog.rdbuf( new LogStreambuf( wxT( "[--]" ), m_fileLog, m_logDebug ) );
			}
		}
		...
		void MainFrame::DoSetMenuBar()
		{
			...
			m_menuFile->Append( Menu_SaveProject, wxString() << TE_PROJECT_SAVE << wxT( "\tCtrl+Shift+S" ) );
			m_menuFile->Append( Menu_SaveFile, wxString() << TE_FILE_SAVE << wxT( "\tCtrl+S" ) );
			m_menuFile->Append( Menu_SaveFileAs, wxString() << TE_FILE_SAVE_AS << wxT( "\tCtrl+Alt+S" ) );
			m_menuFile->AppendSeparator();
		...
		BEGIN_EVENT_TABLE( MainFrame, wxFrame )
			...
			EVT_MENU( Menu_SaveFile, MainFrame::OnSaveFile )
			EVT_MENU( Menu_SaveFileAs, MainFrame::OnSaveFileAs )
		...
		void MainFrame::OnCloseProject( wxCommandEvent & p_event )
		{
			...
			if ( m_projectFrame && m_projectFrame->GetProject()->IsModified() )
			{
