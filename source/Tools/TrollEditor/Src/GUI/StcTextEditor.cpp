...
wxString wxStcTextEditor::DeterminePrefs( wxString const & p_strFilename )
{
	...
	for ( LanguagesConstIterator l_it = m_pContext->Begin() ; l_it != m_pContext->End() && l_strReturn.empty() ; ++l_it )
	{
		l_pCurInfo = * l_it;
...
bool wxStcTextEditor::InitializePrefs( wxString const & p_strName )
{
	...
	for ( LanguagesConstIterator l_it = m_pContext->Begin() ; l_it != m_pContext->End() && ! l_bFound ; ++l_it )
	{
		l_pCurInfo = * l_it;
	...
	if ( l_bFound )
	{
		...
		SetIndent( m_pContext->GetIndentEnable() ? 4 : 0 );
		// others
