...
{
	TrollScriptCompiler::TrollScriptCompiler( Troll::GUI::StcContext * p_context, const String & p_path, bool p_initTree )
		: m_context( NULL )
		, m_textCtrl( NULL )
	...
	// Specific to Troll
		if ( m_initTree )
		{
			...
			va_end( l_argsList );
			GUI::MainFrame::GetInstance()->CreateFreeFunction( make_wxString( p_functionName ), p_returnValue, l_params );
		}

		Troll::GUI::StcContext * l_context = NULL;
	...
	// Specific to Troll
		if ( m_initTree )
		{
			...
			va_end( l_argsList );
			GUI::MainFrame::GetInstance()->CreateClassFunction( make_wxString( p_functionName ), p_returnValue, l_params );
		}
	...
	// Specific to Troll
		...
		if ( m_initTree )
		{
			GUI::MainFrame::GetInstance()->CreateConstant( make_wxString( p_name ), p_type );
		}
	...
	void TrollScriptCompiler::_functionGroup( const String & p_name )
	{
		if ( m_initTree )
		{
			GUI::MainFrame::GetInstance()->CreateFunctionsGroup( make_wxString( p_name ) );
		}
	}
	...
	void TrollScriptCompiler::_constantGroup( const String & p_name )
	{
		if ( m_initTree )
		{
			GUI::MainFrame::GetInstance()->CreateConstantsGroup( make_wxString( p_name ) );
		}
	}
	...
	void TrollScriptCompiler::_initialiseFunctionMap()
	{
		...
		if ( m_initTree )
		{
			GUI::MainFrame::GetInstance()->GetFunctionsList()->CreateFunctionsTree();
			GUI::MainFrame::GetInstance()->GetFunctionsList()->CreateConstantsTree();
		}
	}
}
