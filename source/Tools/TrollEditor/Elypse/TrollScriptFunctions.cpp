	...
	namespace ElypseRW
	{
		...
		TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Ent_SetMaterial )
		{
			...
			if ( MaterialManager::getSingletonPtr()->getByName( l_materialName ) )
			{
				...
				if ( l_entity != nullptr )
				{
					...
					if ( wxGetApp().GetMainFrame()->IsEditingScene() && wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetEntity( make_wxString( l_entity->getName() ) ) != nullptr )
					{
						wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetEntity( make_wxString( l_entity->getName() ) )->SetMaterial( make_wxString( l_materialName ), false );
					}
				}
				else
				{
		...
		TROLL_SCRIPT_FUNCTION_DECLARE( Troll_OvE_SetHeight )
		{
			...
			if ( p_overlay != nullptr )
			{
				...
				TrollOverlay * l_teOv = wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetOverlay( make_wxString( p_overlay->GetName() ) );

				if ( wxGetApp().GetMainFrame()->IsEditingOverlays() && l_teOv != nullptr )
				{
		...
		TROLL_SCRIPT_FUNCTION_DECLARE( Troll_ScN_RotateAround )
		{
			...
			if ( l_node != nullptr )
			{
				...
				if ( l_angle != 0.0 )
				{
					...
					if ( wxGetApp().GetMainFrame()->IsEditingScene() && wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetSceneNode( make_wxString( l_node->getName() ) ) != nullptr )
					{
						wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetSceneNode( make_wxString( l_node->getName() ) )->SetOrientation( l_node->getOrientation(), false );
						wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetSceneNode( make_wxString( l_node->getName() ) )->SetPosition( l_node->getPosition(), false );
					}
				}
			}
			else
		...
		TROLL_SCRIPT_FUNCTION_DECLARE( Troll_ScN_Scale )
		{
			...
			if ( l_node != nullptr )
			{
				...
				if ( l_scale != Vector3( 1.0, 1.0, 1.0 ) )
				{
					...
					if ( wxGetApp().GetMainFrame()->IsEditingScene() && wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetSceneNode( make_wxString( l_node->getName() ) ) != nullptr )
					{
						wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetSceneNode( make_wxString( l_node->getName() ) )->SetScale( l_node->getScale(), false );
					}
				}
			}
			else
		...
		TROLL_SCRIPT_FUNCTION_DECLARE( Troll_ScN_SetScale )
		{
			...
			if ( l_node != nullptr )
			{
				...
				if ( wxGetApp().GetMainFrame()->IsEditingScene() && wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetSceneNode( make_wxString( l_node->getName() ) ) != nullptr )
				{
					wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetSceneNode( make_wxString( l_node->getName() ) )->SetScale( l_scale, false );
				}
			}
			else
			{
		...
		TROLL_SCRIPT_FUNCTION_DECLARE( Troll_ScN_Reset )
		{
			...
			if ( l_node != nullptr )
			{
				...
				if ( wxGetApp().GetMainFrame()->IsEditingScene() && wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetSceneNode( make_wxString( l_node->getName() ) ) != nullptr )
				{
					wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetSceneNode( make_wxString( l_node->getName() ) )->SetScale( Vector3::UNIT_SCALE, false );
					wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetSceneNode( make_wxString( l_node->getName() ) )->SetOrientation( Quaternion::IDENTITY, false );
				}
			}
			else
			{
		...
		TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Cam_ModeWireFrame )
		{
			...
			if ( l_camera != nullptr )
			{
				if ( l_bool )
				{
					if ( wxGetApp().GetMainFrame()->IsEditingScene() && wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetCamera( make_wxString( l_camera->getName() ) ) != nullptr )
					{
						wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetCamera( make_wxString( l_camera->getName() ) )->SetPolygonMode( tcWireFrame );
					}
				}
				else
				{
					if ( wxGetApp().GetMainFrame()->IsEditingScene() && wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetCamera( make_wxString( l_camera->getName() ) ) != nullptr )
					{
						wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetCamera( make_wxString( l_camera->getName() ) )->SetPolygonMode( tcSolid );
					}
				}
			}
			else
		...
		TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Cam_SetPosition )
		{
			...
			if ( l_camera != nullptr )
			{
				...
				if ( wxGetApp().GetMainFrame()->IsEditingScene() && wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetCamera( make_wxString( l_camera->getName() ) ) != nullptr )
				{
					wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetCamera( make_wxString( l_camera->getName() ) )->SetPosition( l_position, false );
				}
			}
			else
			{
		...
		TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Lgh_SetDirection )
		{
			...
			if ( l_light != nullptr )
			{
				...
				if ( wxGetApp().GetMainFrame()->IsEditingScene() && wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetLight( make_wxString( l_light->getName() ) ) != nullptr )
				{
					wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetLight( make_wxString( l_light->getName() ) )->SetDirection( l_sLightDirection, false );
				}
			}
			else
			{
		...
		TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Lgh_SetPosition )
		{
			...
			if ( l_sLight != nullptr )
			{
				...
				if ( wxGetApp().GetMainFrame()->IsEditingScene() && wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetLight( make_wxString( l_sLight->getName() ) ) != nullptr )
				{
					wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetLight( make_wxString( l_sLight->getName() ) )->SetPosition( l_sLightPosition, false );
				}
			}
			else
			{
		...
		TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Lgh_SetDiffuse )
		{
			...
			if ( l_sLight != nullptr )
			{
				...
				if ( wxGetApp().GetMainFrame()->IsEditingScene() && wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetLight( make_wxString( l_sLight->getName() ) ) != nullptr )
				{
					wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetLight( make_wxString( l_sLight->getName() ) )->SetDiffuse( l_sLightDiffuse, false );
				}
			}
			else
			{
		...
		TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Lgh_SetSpecular )
		{
			...
			if ( l_sLight != nullptr )
			{
				...
				if ( wxGetApp().GetMainFrame()->IsEditingScene() && wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetLight( make_wxString( l_sLight->getName() ) ) != nullptr )
				{
					wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetLight( make_wxString( l_sLight->getName() ) )->SetSpecular( l_sLightSpecular, false );
				}
			}
			else
			{
		...
		{
			if ( wxGetApp().GetMainFrame()->GetProject() != nullptr )
			{
				wxGetApp().GetMainFrame()->GetProject()->SaveScripts();
			}
		}
		...
		TROLL_SCRIPT_FUNCTION_DECLARE( Troll_Spc_ImportMesh )
		{
			...
			wxFileDialog l_dialog( nullptr, wxT( "Choisissez un mesh a ouvrir" ), wxEmptyString, wxEmptyString,
			...
			if ( l_dialog.ShowModal() == wxID_OK )
			{
				...
				auto l_index = l_path.find_last_of( wxFileName::GetPathSeparator() );
				...
				File const & l_file = wxGetApp().GetMainFrame()->GetProject()->GetMainScene()->GetDataFile( 0 );
				...
				String l_destination = make_string( wxGetApp().GetMainFrame()->GetProject()->GetPath() + l_file.FileName + wxFileName::GetPathSeparator() ) + l_meshName;
				...
				for ( uint16_t i = 0; i < l_nbSubmeshes; i++ )
				{
					...
					if ( l_material != nullptr )
					{
						if ( l_material->getOrigin() != l_fileName )
						{
							...
							String l_destination = make_string( wxString( wxGetApp().GetMainFrame()->GetProject()->GetPath() ) << l_file.FileName << wxString( wxFileName::GetPathSeparator() ) ) + l_fileName;
						...
						for ( uint16_t j = 0; j < l_nbTechniques; j++ )
						{
							...
							for ( uint16_t k = 0; k < l_nbPasses; k++ )
							{
								...
								for ( uint16_t l = 0; l < l_nbTextureUnits; l++ )
								{
									...
									String l_destination = make_string( wxString( wxGetApp().GetMainFrame()->GetProject()->GetPath() ) << l_file.FileName << wxString( wxFileName::GetPathSeparator() ) ) + l_textureUnit->getTextureName();
