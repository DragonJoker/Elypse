...
namespace General::Files
{
	...
	public:
		bool Initialise();
		void Close();
		CURLcode OpenUrl( Utils::Url const & p_url, size_t ( * p_function )( void *, size_t, size_t, void * ), void * p_data, std::string const & p_postParams = std::string() );

		CURLcode GetStringFromUrl( Utils::Url const & p_url, std::string & p_contents, std::string const & p_postParams = std::string() );
		CURLcode DownloadFile( Utils::Url const & p_url, std::string const & p_filename, std::string const & p_postParams = std::string() );

		std::string GetLastContentType();
		std::string GetContentType( Utils::Url const & p_url, std::string const & p_postParams = std::string() );
