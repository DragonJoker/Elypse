...
namespace Troll
{
	namespace GUI
	{
		...
		String GetKeyFromReg( char const * const p_registerName, char const * const p_keyName )
		{
			...
			{
				returnStatus = RegQueryValueExA( hKey, p_keyName, nullptr, &dwType, ( LPBYTE )&lszValue, &dwSize );
...
#	error Not supported on this platform.
		...
		void ElypseCtrl::DoClose()
		{
			...
			try
			{
				...
				if ( ElypseController::GetSingletonPtr() != nullptr && m_emuse->IsMain() )
				{
		...
		void ElypseCtrl::OnButtonLeftDown( wxMouseEvent & p_event )
		{
			...
			//	{
			//		if (itr->movable != nullptr && itr->movable->getParentSceneNode() != l_manager->getRootSceneNode())
			//		{
		...
		void ElypseCtrl::_initialise()
		{
			...
			if ( osvi.dwMajorVersion >= 6 )
			{
				...
				l_tempPath = m_installPath / "AppData" / "Roaming";
			}
			else if ( osvi.dwMajorVersion > 5 )
			{
				...
				l_tempPath = m_installPath / "AppData" / "LocalLow";
			}
			else
			{
				m_isVista = false;
			...
			m_installPath = l_tempPath / "DragonJoker" / "Elypse";

			if ( !m_fucked )
			{
				Path l_tempString{ m_installPath };
				BOOL l_res;

				if ( _access( l_tempString.c_str(), 0 ) == -1 )
				{
					l_tempPath /= "DragonJoker";
					l_res = CreateDirectoryA( l_tempPath.c_str(), nullptr );
					l_res = CreateDirectoryA( l_tempString.c_str(), nullptr );
				}
		...
		void ElypseCtrl::_showContextMenu( wxPoint const & p_position )
		{
			...
			if ( ScriptEngine::GetContext()->scriptEngine->GetVariable( "editor_currentOverlay" )->get<EMOverlay *>() != nullptr )
			{
