...
BEGIN_TROLL_GUI_NAMESPACE
{
	...
	ElypseCtrl::ElypseCtrl( wxWindow * p_parent, TROLL_PROJECT_NAMESPACE::ProjectSPtr p_project, MainFrame * p_mainFrame, bool p_edit )
...
#	error Not supported on this platform.
	...
	void ElypseCtrl::OnKeyDown( wxKeyEvent & p_event )
	{
		...
		if ( p_event.GetRawKeyCode() == WXK_F9 )
		{
			...
			wxPostEvent( MainFrame::GetInstance(), l_event );
			return;
		}
	...
	void ElypseCtrl::OnNew( wxCommandEvent & p_event )
	{
		...
		if ( l_dialog.ShowModal() == wxID_OK )
		{
			...
			m_newOverlayFrame = new NewOverlayFrame( GUI::MainFrame::GetInstance(), wxPoint( 200, 200 ), l_overlay, l_name );
	...
	void ElypseCtrl::OnDelete( wxCommandEvent & p_event )
	{
		...
		if ( l_overlay != NULL )
		{
			TROLL_PROJECT_2D_NAMESPACE::TrollOverlay * l_teov = GUI::MainFrame::GetInstance()->GetCurrentProject()->GetMainScene()->GetOverlay( make_wxString( l_overlay->GetName() ) );
	...
	void ElypseCtrl::_initialise()
	{
		...
		if ( ! m_fucked )
		{
			...
			try
			{
				m_emuse = new TROLL_ELYPSE_NAMESPACE::TrollInstance( m_installPath , m_plugin );
			}
			catch ( ... )
			{
